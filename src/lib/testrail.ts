  ...
  public isRunToday() {
    return axios({
      ...
      this.lastRunDate = moment
        .unix(response.data.runs[0].created_on)
        .format("MM/DD/YYYY");
      ...
      if (this.lastRunDate === this.currentDate) {
        console.log(
          `Test Run already created today. Posting results to Test Run ID: R${response.data.runs[0].id}`
        );
        return true;
      }
      return false;
